...
function createEvent (name: string, description: string, dateInit: moment.Moment, dateEnd: moment.Moment): void {
    if(!name || !description || !dateInit || !dateEnd){
        console.log("Nem todos os parâmetros foram passados. Tente novamente")
    }

    const diffDateInitAndToday = dateInit.diff(moment(), "seconds");
    const diffDateEndAndToday = dateEnd.diff(moment(), "seconds");

    if(diffDateInitAndToday < 0 && diffDateEndAndToday < 0){
        console.log("A data passada é menor que a data atual")
    }
...
createEvent("Cinema", "Harry Potter", moment("27/08/2020 18:00", "DD/MM/YYYY HH:mm"), moment("27/08/2020 20:30", "DD/MM/YYYY HH:mm"))
information()